 ...
 *@param subkeys sm4子密钥
 *@param b 临时buffer
 *@param r 临时buffer
 *@param dst 128bits plain/crypto text
 *@param src 128bits crypto/plain text
 *@param decrypt enc or dec
 *@return void
 */
func cryptBlock(subkeys []uint32, b []uint32, r []byte, dst, src []byte, decrypt bool) {
	var x uint32
	// copy 128bits data to unsigned  int[4]
	permuteInitialBlock(b, src)
	// 解密操作
	if decrypt {
		// Xi+4=F(Xi,Xi+1,Xi+2,Xi+3,rk31-i)
		for i := 0; i < 8; i++ {
	...
	// Y0=X35,Y1=X34,Y2=X33,Y3=32
	b[0], b[1], b[2], b[3] = b[3], b[2], b[1], b[0]
	// copy encrypted unsigned  int[4] to output 128bits
	permuteFinalBlock(r, b)
	copy(dst, r)
}

/**
 *@description:this function use to set the encrypto key into the subkeys,and then execute key expanding
 *@param key 128bits encrypt key
 *@return void
 */
...
/**
 *@description:t通过证书内容读取M4密钥
 *@param data 证书内容
 *@param pwd 保护SM4密钥的口令
 *@return PEM encoded structure和错误码
 */
...
/**
 *@description:t通过证书文件名称读取SM4密钥
 *@param FileName 证书文件名称
 *@param pwd 保护SM4密钥的口令
 *@return PEM encoded structure和错误码
 */
